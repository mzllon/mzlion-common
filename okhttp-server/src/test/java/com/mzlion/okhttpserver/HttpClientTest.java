package com.mzlion.okhttpserver;

import com.mzlion.okhttpserver.response.HttpResponse;
import org.junit.Test;

import static org.junit.Assert.*;

/**
 * Created by mzlion on 2016/4/17.
 */
public class HttpClientTest {
    @Test
    public void setConnectionTimeout() throws Exception {

    }

    @Test
    public void setReadTimeout() throws Exception {

    }

    @Test
    public void setSSLPathname() throws Exception {

    }

    @Test
    public void setSSLInputStream() throws Exception {

    }

    @Test
    public void setDefaultHeader() throws Exception {

    }

    @Test
    public void setDefaultParams() throws Exception {

    }

    @Test
    public void getOkHttpClient() throws Exception {

    }

    @Test
    public void get() throws Exception {

    }

    @Test
    public void post() throws Exception {

    }

    @Test
    public void postFormData() throws Exception {

    }

    @Test
    public void postRaw() throws Exception {

    }

    @Test
    public void postBin() throws Exception {
    }

}